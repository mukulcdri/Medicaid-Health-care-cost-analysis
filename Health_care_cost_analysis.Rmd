---
title: "Health care cost analysis (Medicaid)"
output: 
...
### **States spending less over the years (Healthier State).**
...
### **Healthier vs unhealthier staes (Classification).**
...
plot(hc_c, labels = states$State, hang = -1, cex = 1.0,
     main = "Correlation between States",
     xlab = "States",
     ylab = "Height") 
...
### **Healthier vs unhealthier staes (Clustering).**

```{r, fig.height=10, fig.width=18}
# k means clustering with normalized values
kc <- kmeans(z, 5)
plot(total_cost_per_cap ~ total_cost, states, col = kc$cluster, pch = 19,cex = 1.5,
     main = "Healthy vs Unhealthy States",
...
axis(side = 1,at = c(150000000,5000000000,10000000000,
                 20000000000, 30000000000, 40000000000 ),
     labels = c("0.15","5.0","10","20", "30","40"))
...
### **Diseases resposible for the mess.**
...
# plotting
    ...
    top_diseases %>% ggplot(aes(Dis, total_cost, size = total_cost)) +
     ...
     scale_size_continuous(range = c(1,20)) + 
     labs(title = "Disease prevalence in country", x = "Disease_name",
          y = "Total cost (billions)") +
     theme(title = black.bold.italic.16.text,
    axis.title =   black.bold.italic.16.text,
...
### **Prescription vs Generic.**

```{r, fig.height=2, fig.width=4}
# changing the name of columns in top10drugs_dis_generic
   names(top10drugs_dis_generic)[names(top10drugs_dis_generic) ==
                                   "tot_amt_reim"] <- "Pres_cost"
   names(top10drugs_dis_generic)[names(top10drugs_dis_generic) ==
...
comp_Prod <- gather(top10drugs_dis_generic, Drug, Cost, c("Pres_cost","Gen_cost"))
 ...
 # Plotting  box plot showing difference
   ggboxplot(comp_Prod, x = "Drug", y = "Cost", color = "Drug",
                  palette = "jco", add = "jitter") +
     stat_compare_means(method = "t.test", label.y = 20, size = 2.5) +
          labs(title = "Prescription vs Generic", x = "Drugs",
          y = "Total cost (billions)") +
     theme(title = black.bold.italic.6.text,
    axis.title =   black.bold.italic.6.text,
...
theme(title = black.bold.italic.6.text, axis.title = black.bold.italic.6.text) +
